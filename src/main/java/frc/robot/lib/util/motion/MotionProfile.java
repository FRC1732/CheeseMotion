...
public class MotionProfile {
	 ...
	 * the *first* instance of this position.
	 * 
	 * @param pos
	...
	public Optional<MotionState> firstStateByPos(double pos) {
		for (MotionSegment s : mSegments) {
			if (s.containsPos(pos)) {
				if (Util.epsilonEquals(s.end().pos(), pos, MotionUtil.kEpsilon)) {
	 ...
	 * whole segments and also shortens any segments containing t.
	 * 
	 * @param t
	 *            The query time.
	 */
	 ...
	 * of length 0 that starts and ends at initial_state).
	 * 
	 * @param initial_state
	 ...
	 * is appended to the previous last state.
	 * 
	 * @param acc
	 ...
	 *         sum of absolute distances of all segments, so a reversing profile
