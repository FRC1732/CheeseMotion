...
public class MotionProfileGoal {
	 ...
	 * greater than the maximum? This enum allows a user to specify a preference on
	 ...
	 * unwilling to violate the max_abs_vel (for example, there is an obstacle in
	 * front of us - slam the brakes harder than we'd like in order to avoid hitting
	 * it).
	 * 
	 * VIOLATE_MAX_ABS_VEL - If the max velocity is just a general guideline and not
	 * a hard performance limit, it's better to slightly exceed it to avoid skidding
	 * wheels.
	 */
