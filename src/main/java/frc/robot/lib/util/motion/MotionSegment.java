...
public class MotionSegment {
	...
	public boolean isValid() {
		if (!Util.epsilonEquals(start().acc(), end().acc(), MotionUtil.kEpsilon)) {
		...
		if (Math.signum(start().vel()) * Math.signum(end().vel()) < 0.0
				&& !Util.epsilonEquals(start().vel(), 0.0, MotionUtil.kEpsilon)
				&& !Util.epsilonEquals(end().vel(), 0.0, MotionUtil.kEpsilon)) {
