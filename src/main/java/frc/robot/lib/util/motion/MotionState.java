package frc.robot.lib.util.motion;
...
public class MotionState {
	 ...
	 * @return A MotionState that is a valid predecessor (if t<=0) or successor (if
	 *         t>=0) of this state (with the specified accel).
	 */
	 ...
	 * be at pos. This is an inverse of the extrapolate() method.
	 * 
	 * @param pos
	...
	public double nextTimeAtPos(double pos) {
		if (Util.epsilonEquals(pos, this.pos, MotionUtil.kEpsilon)) {
			// Already at pos.
			return t;
		}
		if (Util.epsilonEquals(acc, 0.0, MotionUtil.kEpsilon)) {
			// Zero acceleration case.
			final double delta_pos = pos - this.pos;
			if (!Util.epsilonEquals(vel, 0.0, MotionUtil.kEpsilon) && Math.signum(delta_pos) == Math.signum(vel)) {
	...
	public boolean equals(Object other) {
		return (other instanceof MotionState) && equals((MotionState) other, MotionUtil.kEpsilon);
	}
	 ...
	 * nominal tolerance, but acceleration may be different).
	 */
	public boolean coincident(MotionState other) {
		return coincident(other, MotionUtil.kEpsilon);
	}

	/**
	 * Checks if two MotionStates are coincident (t, pos, and vel are equal within a
	 * specified tolerance, but acceleration may be different).
	 */
